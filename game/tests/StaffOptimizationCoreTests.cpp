...
int main(){try{
  ...
  p.assignments={{11,2,100,160}};require(validateAssignmentPlan(s,p),"rejected valid assignment");
  ...
  require(a==b,"fallback scheduler is not deterministic");require(a.assignments.size()==2,"did not assign feasible tasks");require(a.assignments.front().taskId==11,"critical task not prioritized");require(validateAssignmentPlan(f,a),"fallback plan invalid");
