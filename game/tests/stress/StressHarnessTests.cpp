...
#endif
  const auto hex = hh::stress::configFromEnvironment(1);
  assert(hex.seed == 42);

#if defined(_WIN32)
  _putenv_s("HH_STRESS_SEED", "987654321");
#else
  setenv("HH_STRESS_SEED", "987654321", 1);
#endif
  const auto decimal = hh::stress::configFromEnvironment(1);
  assert(decimal.seed == 987654321ULL);
