...
inline std::uint64_t parseSeed(std::string_view text) {
  if (text.empty() || text.front() == '-')
    throw std::invalid_argument("HH_STRESS_SEED must be an unsigned integer");
  std::size_t consumed = 0;
  const auto value = std::stoull(std::string(text), &consumed, 0);
  if (consumed != text.size())
    throw std::invalid_argument("HH_STRESS_SEED contains trailing characters");
...
inline Config configFromEnvironment(std::uint64_t fallbackSeed) {
  ...
  if (const char *seed = std::getenv("HH_STRESS_SEED"); seed && *seed)
    config.seed = parseSeed(seed);
  if (const char *scenario = std::getenv("HH_STRESS_SCENARIO"); scenario && *scenario)
...
public:
  ...
  std::uint64_t bounded(std::uint64_t exclusiveUpperBound) {
    if (exclusiveUpperBound == 0)
      throw std::invalid_argument("bounded upper bound must be non-zero");
    const auto threshold = static_cast<std::uint64_t>(-exclusiveUpperBound) % exclusiveUpperBound;
    for (;;) {
  ...
  bool chance(std::uint32_t numerator, std::uint32_t denominator) {
    if (denominator == 0 || numerator > denominator)
      throw std::invalid_argument("invalid chance fraction");
...
public:
  ...
  void push(std::string action) {
    ...
    if (entries_.size() == capacity_)
      entries_.pop_front();
    entries_.push_back(std::move(action));
  }
...
struct RunContext {
  ...
  [[noreturn]] void fail(std::string_view invariant,
    ...
    std::ostringstream out;
    out << "stress failure suite=" << suite
        << " seed=0x" << std::hex << config.seed << std::dec
        << " scenario=" << (config.scenario.empty() ? "all" : config.scenario)
        << " phase=" << (phase.empty() ? "unspecified" : phase)
        << " tick_or_day=" << tickOrDay
        << " invariant=" << invariant;
    if (!stateHash.empty())
      out << " hash=" << stateHash;
    const auto recent = trace.dump();
    if (!recent.empty())
      out << " trace=[" << recent << ']';
    std::cerr << out.str() << '\n';
