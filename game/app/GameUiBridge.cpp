...
makeGameUiSnapshotSource(const hh::game::Simulation& simulation,
  ...
  for (const auto& room : view.rooms) {
    ...
    entity.fields = {{"Status", roomStatusText(room.status)}, {"Floor", std::to_string(room.floor)}, {"Cleanliness", percentage(room.cleanliness)}, {"Condition", percentage(room.condition)}, {"Nightly rate", cents(room.nightlyRateCents)}, {"Reachable", room.reachable ? "Yes" : "No"}, {"Closed", room.closed ? "Yes" : "No"}};
    if (!room.reachable) { entity.diagnostics.push_back({"ROOM_UNREACHABLE", "No route to the guest entrance", room.id, 0}); out.alerts.push_back({alertId(0x1100000000000000ULL, room.id), AlertSeverity::Critical, room.id, "ROOM_UNREACHABLE", "Room has no guest-access route", 0, false}); }
    if (room.status == hh::game::RoomStatus::OutOfOrder) { entity.diagnostics.push_back({"ROOM_OUT_OF_ORDER", "Room is out of service", room.id, 0}); out.alerts.push_back({alertId(0x1200000000000000ULL, room.id), AlertSeverity::Warning, room.id, "ROOM_OUT_OF_ORDER", "Room is out of service", 0, false}); }
    if (room.closed) entity.diagnostics.push_back({"ROOM_CLOSED", "Room is closed by player policy", room.id, 0});
    out.entities.push_back(std::move(entity));
    const auto clean = static_cast<std::int64_t>(std::lround(room.cleanliness * 10.0));
    cleanliness.samples.push_back({room.id, clean, percentage(room.cleanliness), room.cleanliness < 70.0 ? "Needs cleaning" : "Clean"});
    const auto condition = static_cast<std::int64_t>(std::lround(room.condition * 10.0));
    roomQuality.samples.push_back({room.id, condition, percentage(room.condition), room.condition < 60.0 ? "Low condition" : "Good condition"});
    maintenance.samples.push_back({room.id, condition, percentage(room.condition), room.condition < 60.0 ? "Maintenance attention" : "Serviceable"});
    roomStatus.samples.push_back({room.id, static_cast<std::int64_t>(room.status), roomStatusText(room.status), roomStatusText(room.status)});
    hashValue(revision, room.id); hashValue(revision, static_cast<std::uint64_t>(room.status)); hashValue(revision, static_cast<std::uint64_t>(room.nightlyRateCents)); hashValue(revision, static_cast<std::uint64_t>(std::lround(room.cleanliness * 100.0))); hashValue(revision, static_cast<std::uint64_t>(std::lround(room.condition * 100.0))); hashValue(revision, room.reachable ? 1U : 0U); hashValue(revision, room.closed ? 1U : 0U);
  }

  for (const auto& person : view.people) {
    UiEntitySnapshot entity; entity.id = person.id; entity.kind = inspectorKind(person.kind); entity.title = person.name;
    entity.fields = {{"Role", personKindText(person.kind)}, {"State", personStateText(person.state)}, {"Goal", person.goal}, {"Floor", std::to_string(person.position.floor)}, {"Fatigue", percentage(person.fatigue)}, {"Satisfaction", percentage(person.satisfaction)}, {"Queue wait", std::to_string(person.queueWaitSeconds) + " s"}, {"Travel", std::to_string(person.travelSeconds) + " s"}};
    if (person.kind != hh::game::PersonKind::Guest) { entity.fields.push_back({"Shift", std::to_string(person.shiftStartHour) + ":00–" + std::to_string(person.shiftEndHour) + ":00"}); entity.fields.push_back({"On shift", person.onShift ? "Yes" : "No"}); entity.fields.push_back({"Hourly wage", cents(person.hourlyWageCents)}); }
    out.entities.push_back(std::move(entity));
    if (person.kind == hh::game::PersonKind::Guest) { guestSatisfactionTotal += person.satisfaction; ++guestSatisfactionCount; satisfaction.samples.push_back({person.id, static_cast<std::int64_t>(std::lround(person.satisfaction * 10.0)), percentage(person.satisfaction), person.satisfaction < 60.0 ? "Dissatisfied" : "Satisfied"}); }
    else { const bool busy = person.state == hh::game::PersonState::Working || person.state == hh::game::PersonState::Traveling; staffUtilization.samples.push_back({person.id, busy ? 1000 : 0, busy ? "Active" : "Not active", personStateText(person.state)}); if (person.onShift) ++out.operations.activeStaff; ++out.operations.scheduledStaff; if (person.fatigue >= 75.0) ++out.operations.fatiguedStaff; }
    if (person.queueWaitSeconds > 0) queueWait.samples.push_back({person.id, person.queueWaitSeconds, std::to_string(person.queueWaitSeconds) + " s", person.queueWaitSeconds >= 300 ? "Long wait" : "Waiting"});
    hashValue(revision, person.id); hashValue(revision, static_cast<std::uint64_t>(person.state)); hashValue(revision, static_cast<std::uint64_t>(std::lround(person.satisfaction * 100.0))); hashValue(revision, static_cast<std::uint64_t>(person.queueWaitSeconds));
  }

  if (guestSatisfactionCount > 0) out.hud.satisfactionPermille = static_cast<int>(std::lround(guestSatisfactionTotal * 10.0 / static_cast<double>(guestSatisfactionCount)));

  std::map<hh::frontend::EntityId, int> taskCountByTarget;
  for (const auto& task : view.tasks) {
    UiEntitySnapshot entity; entity.id = task.id; entity.kind = InspectorKind::Task; entity.title = taskKindText(task.kind) + " #" + std::to_string(task.id); entity.fields = {{"Status", taskStatusText(task.status)}, {"Target", std::to_string(task.targetId)}, {"Employee", std::to_string(task.employeeId)}, {"Remaining", oneDecimal(task.workRemainingSeconds) + " s"}};
    if (task.status == hh::game::TaskStatus::Blocked) { const auto code = task.blockedReason.empty() ? std::string{"TASK_BLOCKED"} : task.blockedReason; const auto message = task.blockedReason.empty() ? std::string{"Task is blocked"} : task.blockedReason; entity.diagnostics.push_back({code, message, task.targetId, 0}); out.alerts.push_back({alertId(0x2100000000000000ULL, task.id), AlertSeverity::Warning, task.targetId != 0 ? task.targetId : task.id, code, message, 0, false}); }
    out.entities.push_back(std::move(entity));
    out.operations.rows.push_back({task.id, taskArea(task.kind), taskKindText(task.kind), taskStatusText(task.status), task.status == hh::game::TaskStatus::Blocked ? 3 : 1, task.blockedReason, 0});
    if (task.kind == hh::game::TaskKind::Turnover && task.status != hh::game::TaskStatus::Completed) ++out.operations.housekeepingBacklog;
    if (task.kind == hh::game::TaskKind::Repair && task.status != hh::game::TaskStatus::Completed) ++out.operations.engineeringOpenOrders;
    if (task.status != hh::game::TaskStatus::Completed) ++taskCountByTarget[task.targetId];
    hashValue(revision, task.id); hashValue(revision, static_cast<std::uint64_t>(task.status)); hashText(revision, task.blockedReason);
  }
  for (const auto& [target, count] : taskCountByTarget) openTasks.samples.push_back({target, count, std::to_string(count) + " open", count > 2 ? "High task density" : "Open tasks"});

  UiEntitySnapshot inventoryEntity; inventoryEntity.id = 0xF000000000000001ULL; inventoryEntity.kind = InspectorKind::Inventory; inventoryEntity.title = "Property inventory"; inventoryEntity.fields = {{"Linen", std::to_string(view.inventory.linen)}, {"Towels", std::to_string(view.inventory.towels)}, {"Amenities", std::to_string(view.inventory.amenities)}, {"Chemicals", std::to_string(view.inventory.chemicals)}, {"Parts", std::to_string(view.inventory.parts)}}; out.entities.push_back(std::move(inventoryEntity));
  for (const auto& order : view.supplyOrders) if (!order.delivered) ++out.operations.incomingOrders;
  for (const auto& stack : logistics.inventory) if (stack.item == "clean_linen") out.operations.cleanLinenUnits += stack.quantity - stack.reservedQuantity;
  for (const auto& order : logistics.purchaseOrders) { if (order.state != hh::game::PurchaseOrderState::Completed && order.state != hh::game::PurchaseOrderState::Cancelled) { ++out.operations.incomingOrders; const auto code = blockReasonCode(order.blockedReason); out.operations.rows.push_back({order.id, OperationArea::Logistics, "Purchase order · " + order.item, code.empty() ? "In progress" : "Blocked", code.empty() ? 1 : 3, code, 0}); if (!code.empty()) ++out.operations.blockedInventoryMoves; } }
  for (const auto& move : logistics.stockMoves) { if (move.completed) continue; const auto code = blockReasonCode(move.blockedReason); out.operations.rows.push_back({move.id, OperationArea::Logistics, "Inventory move · " + move.item, code.empty() ? "In progress" : "Blocked", code.empty() ? 1 : 3, code, 0}); if (!code.empty()) ++out.operations.blockedInventoryMoves; }
  for (const auto& order : food.orders) { if (terminalFoodStage(order.stage)) continue; ++out.operations.foodTickets; const auto code = foodBlockCode(order.blockReason); out.operations.rows.push_back({order.id, OperationArea::FoodAndBeverage, "F&B ticket #" + std::to_string(order.id), foodStageText(order.stage), code.empty() ? 1 : 3, code, order.ageSeconds}); if (!code.empty()) out.alerts.push_back({alertId(0x3100000000000000ULL, order.id), AlertSeverity::Warning, order.id, code, "Food-service order blocked", 0, false}); }
  for (const auto& booking : events.bookings) { if (booking.phase != hh::game::EventPhase::Completed && booking.phase != hh::game::EventPhase::Cancelled) { ++out.operations.eventCount; out.operations.rows.push_back({booking.id, OperationArea::Events, "Event #" + std::to_string(booking.id), "Active", 1, "", 0}); } }
  for (const auto& amenity : amenities.amenities) { out.operations.amenityCapacityUsed += amenity.active; out.operations.amenityCapacityTotal += amenity.capacity; UiEntitySnapshot entity; entity.id = amenity.id; entity.kind = InspectorKind::ServiceVenue; entity.title = "Amenity #" + std::to_string(amenity.id); entity.fields = {{"Open", amenity.open ? "Yes" : "No"}, {"Capacity", std::to_string(amenity.active) + "/" + std::to_string(amenity.capacity)}, {"Cleanliness", percentage(static_cast<double>(amenity.cleanliness) / 100.0)}, {"Condition", percentage(static_cast<double>(amenity.condition) / 100.0)}, {"Price", cents(amenity.priceCents)}}; if (amenity.cleaningRequired) entity.diagnostics.push_back({"AMENITY_CLEANING_REQUIRED", "Cleaning required", amenity.id, 0}); if (amenity.maintenanceRequired) entity.diagnostics.push_back({"AMENITY_MAINTENANCE_REQUIRED", "Maintenance required", amenity.id, 0}); out.entities.push_back(std::move(entity)); }

  const auto activeRows = static_cast<int>(out.operations.rows.size());
  const auto blockedRows = static_cast<int>(std::count_if(out.operations.rows.begin(), out.operations.rows.end(), [](const OperationRow& row) { return !row.reasonCode.empty(); }));
  out.operations.serviceLevelPermille = activeRows == 0 ? 1000 : std::max(0, 1000 - (blockedRows * 1000 / activeRows));
  ...
  if (!satisfaction.samples.empty()) out.overlays.push_back(std::move(satisfaction));
  ...
  if (!openTasks.samples.empty()) out.overlays.push_back(std::move(openTasks));
  if (!staffUtilization.samples.empty()) out.overlays.push_back(std::move(staffUtilization));
  if (!queueWait.samples.empty()) out.overlays.push_back(std::move(queueWait));
  ...
  hashValue(revision, context.cutaway ? 1U : 0U);
