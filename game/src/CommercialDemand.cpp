...
int ewma(int previous, int sample) {
  constexpr int alphaBasisPoints = 1500;
  return std::clamp((previous * (10000 - alphaBasisPoints) + sample * alphaBasisPoints + 5000) /
                        10000,
                    0, 10000);
}
...
void CommercialDemand::applyReview(const ReviewSignal &review) {
  ...
  snapshot_.cleanlinessReputationBasisPoints =
      ewma(snapshot_.cleanlinessReputationBasisPoints, review.cleanlinessBasisPoints);
...
CommercialCommandResult CommercialDemand::startCampaign(const MarketingCampaign &campaign,
  ...
  if (std::any_of(snapshot_.campaigns.begin(), snapshot_.campaigns.end(), [&](const auto &existing) {
        return existing.id == campaign.id;
      }))
    return {false, "DUPLICATE_CAMPAIGN_ID", 0};
  snapshot_.campaigns.push_back(campaign);
  std::sort(snapshot_.campaigns.begin(), snapshot_.campaigns.end(), [](const auto &a, const auto &b) {
    return a.id < b.id;
  });
...
ContractAcceptanceResult CommercialDemand::acceptContract(
  ...
  if (contract.id == 0 || contract.startDay < 0 || contract.endDay < contract.startDay ||
      contract.minimumRoomNights < 0 || contract.maximumRoomNights < contract.minimumRoomNights ||
  ...
  if (std::any_of(snapshot_.contracts.begin(), snapshot_.contracts.end(), [&](const auto &existing) {
        return existing.contract.id == contract.id;
      }))
    return {false, "DUPLICATE_CONTRACT_ID", 0, false};

  const bool feasible = feasibility.availableRoomNights >= contract.minimumRoomNights &&
  ...
  std::sort(snapshot_.contracts.begin(), snapshot_.contracts.end(), [](const auto &a, const auto &b) {
    return a.contract.id < b.contract.id;
  });
  return {true, feasible ? "OK" : "ACCEPTED_WITH_EXPLICIT_RISK", contract.id, !feasible};
}
...
std::string CommercialDemand::save() const {
  ...
  out << "HHCOMM 1 " << snapshot_.overallReputationBasisPoints << ' '
      << snapshot_.serviceReputationBasisPoints << ' '
      << snapshot_.cleanlinessReputationBasisPoints << ' '
      << snapshot_.valueReputationBasisPoints << ' ' << snapshot_.campaigns.size();
  for (const auto &campaign : snapshot_.campaigns) {
...
CommercialDemand CommercialDemand::load(std::string_view data) {
  ...
  in >> magic >> version >> result.snapshot_.overallReputationBasisPoints >>
      ...
      result.snapshot_.valueReputationBasisPoints >> count;
  if (!in || magic != "HHCOMM" || version != 1 || count > 100000)
  ...
  for (std::size_t i = 0; i < count; ++i) {
    ...
    in >> c.id >> c.startDay >> c.endDay >> c.minimumRoomNights >> c.maximumRoomNights >>
        c.negotiatedRateCents >> c.requiredVenueCapacity >> c.requiredServiceUnits >>
        c.cancellationPenaltyCents >> c.paymentDelayDays >> accepted.acceptedRisk;
    if (!in)
