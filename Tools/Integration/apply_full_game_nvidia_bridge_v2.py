...
# The current integration head already composes optimization/ at root and links
# hh_game PRIVATE to hh_optimization. Only add the bridge source and test access.
replace_once(
    "game/CMakeLists.txt",
...
new_assignment_block = r'''    for (auto &task : tasks)
    ...
    // Immediate front-desk service stays on the native path. Checkout is
    // deliberately serviced before older check-in backlog.
    for (auto &task : tasks)
...
print("Applied Hotel Haven NVIDIA scheduler bridge v2")
