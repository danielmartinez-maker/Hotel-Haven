...
def test_high_detail_staff_stay_inside_character_face_budget():
 names=['Bellhop Man','Bellhop Woman','Chef Man','Chef Woman','Hotel Manager Woman']
 for index,name in enumerate(names,34):
  scene=build_character(name,index)
  faces=sum(len(g.faces) for g in scene.geometry.values())
  assert faces<=2500,(name,faces)

def test_guest_silhouettes_are_not_all_identical():
 names=['Guest Businessman','Guest Backpacker Woman','Guest Child Boy','Guest Elderly Woman','Guest Luxury Traveler Man']
 signatures=[]
 for index,name in enumerate(names):
  scene=build_character(name,index)
  bounds=scene.bounds
  ext=np.round(bounds[1]-bounds[0],3)
