...
from amenity_decor_factory import build_asset, sign
...
def generate_package(manifest_path:Path|str,output_dir:Path|str,source_path='Tools/ArtGeneration/batch09_generate.py'):
    ...
    for group in data['groups']:
        ...
        for asset_id,name,subcategory,mat,profile,animation_set,anchors in group['assets']:
            # Restaurant Menu Stand has dedicated freestanding sign geometry; route it explicitly
            # rather than letting the generic amenity/decor dispatcher treat it as an unknown prop.
            scene=sign(name,mat) if 'Menu Stand' in name else build_asset(name,mat); scene.units='meters'; p=output_dir/f'{asset_id}.glb'; p.write_bytes(scene.export(file_type='glb')); out.append(p)
            arch=profile=='P_ARCH_STATIC'; small=profile=='P_SMALL_PROP'; side={'schema':1,'asset_id':asset_id,'asset_type':'StaticMeshAsset','source':source_path,'units':'meters','lod_policy':'lod_architecture' if arch else ('lod_small_prop' if small else 'lod_furniture'),'collision_policy':'none_or_simple_proxy' if small else 'simple_proxy','cutaway_policy':'normal','material_slots':[mat],'tags':['batch_09','architectural_diorama_realism',family,subcategory],'dependencies':[],'milestone':'batch_09','source_revision':2,'metadata_revision':2}
