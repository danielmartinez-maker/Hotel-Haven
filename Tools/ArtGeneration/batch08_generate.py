...
def generate_package(manifest_path:Path|str,output_dir:Path|str,source_path='Tools/ArtGeneration/batch08_generate.py'):
    ...
    for group in data['groups']:
        ...
        for asset_id,name,subcategory,mat,profile,animation_set,anchors in group['assets']:
            ...
            small=profile=='P_SMALL_PROP'; side={'schema':1,'asset_id':asset_id,'asset_type':'StaticMeshAsset','source':source_path,'units':'meters','lod_policy':'lod_small_prop' if small else 'lod_furniture','collision_policy':'none_or_simple_proxy' if small else 'simple_proxy','cutaway_policy':'normal','material_slots':[mat],'tags':['batch_08','architectural_diorama_realism',family,subcategory],'dependencies':[],'milestone':'batch_08','source_revision':1,'metadata_revision':1}
