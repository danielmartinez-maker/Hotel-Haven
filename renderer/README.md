...
The standalone renderer demo consumes `RenderScene` presentation DTOs and remains procedural. In the integrated root build, `game/app/WorldView.cpp` translates authoritative simulation/world state into both box and mesh render items; the renderer must not infer room validity or mutate gameplay state.\n\n### Integrated cooked-asset runtime\n\nThe shipping `hotel_haven` client resolves its visual dependency set once at startup through `RuntimeWorldAssets` and `RuntimeAssetRegistry`. The current world presentation binds 49 cooked assets: room/construction props plus all 30 guest character variants and receptionist, housekeeper and maintenance variants. `loadDirectorySubset` decodes only those required GLBs atomically, while `hh_runtime_asset_library_audit` deliberately loads the complete 500-asset package in CI. Skinned assets currently render their validated bind-pose geometry; skeletal animation playback remains a later runtime layer. Procedural geometry is retained as a fallback for tests and incomplete asset packages.
