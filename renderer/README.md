...
The shipping `hotel_haven` client resolves its visual dependency set once at startup through `RuntimeWorldAssets` and `RuntimeAssetRegistry`. The current world presentation binds 59 cooked assets: room/construction props plus all 30 guest character variants and receptionist, housekeeper and maintenance variants. `loadDirectorySubset` decodes only those required GLBs atomically, while `hh_runtime_asset_library_audit` deliberately loads the complete 500-asset package in CI. Skinned assets currently render their validated bind-pose geometry; skeletal animation playback remains a later runtime layer. Procedural geometry is retained as a fallback for tests and incomplete asset packages.
