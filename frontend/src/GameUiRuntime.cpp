...
void GameUiRuntime::update(const SimulationSnapshot& source) { if (snapshotBuildCount_ != 0 && source.revision == snapshot_.revision && source.hud.cutaway == snapshot_.hud.cutaway) return; snapshot_ = buildGameUiSnapshot(source); ++snapshotBuildCount_; }
